...
local o = {
    save_period = 8  -- updates the bookmark in the '.mpv.watch_later' file periodically.
}
...
function M.get_file_name(idx)
    if idx > #pl_list then
        return ""
    end
    return pl_list[idx]:match("/([^/]+)$") or ""
end

function M.ld_mark()
    local file = io.open(mk_path, "r")
    if file == nil then
        print("can not open watch_later file")
        return false
    end
    pl_name = file:read()
    if pl_name == nil then
...
local timeout = 15 -- Set initial timeout to 10 seconds
function M.wait_jump()
    timeout = timeout - 1
    if(timeout < 1) then
        M.wait_jump_timer:kill()
        M.unbind_key()
        M.save_mark()  -- Save bookmark after the countdown
    end
    local msg = wait_msg.."".."\n".."at "..(math.modf(pl_percent*10)/10).."% | Continue? [y/N]-->"..(timeout < 10 and "0" or "")..timeout
    M.show(msg, 1000)
end
...
function M.exe()
    ...
    if(c_idx == pl_idx) then
        ...
        wait_msg = M.get_file_name(pl_idx)
